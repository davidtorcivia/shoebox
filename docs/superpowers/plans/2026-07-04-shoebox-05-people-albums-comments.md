...
1. **Nickname:** the locked mockup shows a `"Grandma"` nickname line, but `people` has no nickname column (Contract 1) and the schema may not be extended. **Resolution: skip the nickname line entirely; render `person.name` only.** The `.who` column keeps the same top-alignment/stats-lock geometry without it.
